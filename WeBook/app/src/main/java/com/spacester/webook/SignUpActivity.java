    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.signUp).setOnClickListener(v -> {
            ...
            if (mEmail.isEmpty()){
                ...
                Query emailQuery = FirebaseDatabase.getInstance().getReference().child("data").orderByChild("email").equalTo(mEmail);
                emailQuery.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        if (snapshot.getChildrenCount()>0){
                            ...
                            Query usernameQuery = FirebaseDatabase.getInstance().getReference().child("data").orderByChild("username").equalTo(mUsername);
    ...
    private void register(String mEmail, String mPassword, String mName, String mUsername) {
        mAuth.createUserWithEmailAndPassword(mEmail, mPassword).addOnCompleteListener(task -> {
            if (task.isSuccessful()){
                ...
                FirebaseDatabase.getInstance().getReference("data").child(userId).setValue(hashMap).addOnCompleteListener(task1 -> {
